//this is designed to land a small manned lander on the Mun
//tested from a 15km equitorial orbit
//
when ship:body = body("Mun") then {
	run calc.ks.
	run out.ks.
	preserve.
}
...
until status = "LANDED"{
	...
	when verticalspeed < -2 then {
		when airspeed <= 3 then{
			if actTWR >= .99{set mythrot to mythrot - .001. lock throttle to mythrot.}
			else if actTWR <= .97 { if throttle = 1 {} else set mythrot to mythrot + .001. lock throttle to mythrot.}
		}
		when airspeed >= 4 then{
			if actTWR >= 1.1{set mythrot to mythrot - .001. lock throttle to mythrot.}
			else if actTWR <= 1.08 { if throttle = 1 {} else set mythrot to mythrot + .001. lock throttle to mythrot.}
		}
	}
}
unlock steering.
